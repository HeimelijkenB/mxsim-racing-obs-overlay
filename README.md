...
**Current version:** 2.0.13 (see [`docs/CHANGELOG.txt`](docs/CHANGELOG.txt)).
...
2. Download **`MxSimRacingOBSOverlay-v2.0.13-Win64.exe`** (portable; rename to `MxSimRacingOBSOverlay.exe` if you like) and/or **`MxSimRacingOBSOverlay-v2.0.13-Setup.exe`** from the latest release assets.
...
2. **Installer:** install [Inno Setup 6](https://jrsoftware.org/isdl.php), then run **`BUILD_INSTALLER.bat`**.  
   Output: `release\MxSimRacingOBSOverlay-v2.0.13-Setup.exe` (version may change in future tags).  
...
Create a **Release** tagged `v2.0.13` (or the current version) and attach the built portable **`MxSimRacingOBSOverlay-v2.0.13-Win64.exe`** (copy from `dist\MxSimRacingOBSOverlay.exe`) and optionally **`release\MxSimRacingOBSOverlay-v2.0.13-Setup.exe`** after building locally.
